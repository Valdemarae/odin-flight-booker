class BookingsController < ApplicationController
  ...
  def create
    ...
    if @booking.save
      @booking.passangers.each do |p| 
        p.tickets.create(booking_id: @booking.id, code: rand(10**5))
        PassangerMailer.with(user: p, url: booking_url(@booking.id)).booking_confirmation.deliver_now
      end
